...
class ResumeApiTestViews(BaseCoursewareTests, CompletionWaffleTestMixin):
    ...
    def test_resume_no_completion(self):
        response = self.client.get(self.url)
        assert response.status_code == 200
        assert response.data['block'] is None
        assert response.data['unit'] is None
        assert response.data['section'] is None
    ...
    def test_resume_with_completion(self):
        ...
        response = self.client.get(self.url)
        assert response.status_code == 200
        assert response.data['block'] == str(self.unit.location)
        assert response.data['unit'] == str(self.unit.location)
        assert response.data['section'] == str(self.sequence.location)
